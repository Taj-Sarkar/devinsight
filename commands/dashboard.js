...
async function dashboardCommand(options = {}) {
  ...
  if (lastCommit) {
    ...
    gitTable.push(
      ['Last Commit', timeAgo],
      ['Last Commit Message', lastCommit.message.substring(0, 45) + (lastCommit.message.length > 45 ? '...' : '')],
  ...
  if (lastCommitFiles.files.length > 0) {
    console.log(chalk.white(`Files Modified: ${chalk.green(lastCommitFiles.files.length)}`));
    console.log(chalk.white(`Lines Added: ${chalk.green('+' + lastCommitFiles.additions)}`));
    console.log(chalk.white(`Lines Removed: ${chalk.red('-' + lastCommitFiles.deletions)}`));
    ...
    if (lastCommitFiles.files.length > 10) {
      console.log(chalk.gray(`  ... and ${lastCommitFiles.files.length - 10} more`));
    }
  } else {
