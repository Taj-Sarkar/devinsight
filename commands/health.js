...
const { hasPackageJson, getDependenciesSummary, hasScripts } = require('../utils/dependencyParser');
...
async function healthCommand() {
  ...
  // ===== CHECK: README File =====
  const hasReadme = await checkFileExists(rootPath, ['README.md', 'README.txt', 'readme.md']);
  if (!hasReadme) {
    warnings.push({
      type: 'warning',
  ...
  const hasTests = await checkDirectoryExists(rootPath, ['test', 'tests', '__tests__', 'spec']);
  if (!hasTests) {
    warnings.push({
      type: 'warning',
  ...
  if (!hasPackage) {
    ...
    if (depSummary.totalCount > 100) {
      ...
      suggestions.push(`You have ${depSummary.totalCount} dependencies - consider periodic audits`);
    } else {
  ...
  // ===== CHECK: License File =====
  const hasLicense = await checkFileExists(rootPath, ['LICENSE', 'LICENSE.md', 'LICENSE.txt', 'license.md']);
  if (!hasLicense) {
  ...
  // ===== CHECK: Documentation =====
  const hasDocsDir = await checkDirectoryExists(rootPath, ['docs', 'documentation', 'doc']);
  if (hasDocsDir) {
  ...
  if (hasCICD) {
    ...
    suggestions.push('Consider setting up CI/CD for automated testing and deployment');
  }
  ...
  if (warnings.length > 0) {
    ...
    warnings.forEach((warning, index) => {
      const icon = warning.type === 'error' ? '❌' : warning.type === 'warning' ? '⚠️' : 'ℹ️';
  ...
  const healthScore = totalChecks > 0 ? Math.round((passed.length / totalChecks) * 100) : 0;
  ...
  console.log(scoreColor.bold(`${scoreEmoji} ${healthScore}%`) + chalk.white(` (${passed.length}/${totalChecks} checks passed)`));
  ...
  if (healthScore === 100) {
    ...
    console.log(chalk.red('\n⚠️  Needs attention. Please review the warnings above.\n'));
  }
}
