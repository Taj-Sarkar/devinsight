...
const { scanDirectory, detectLanguages, getLargestDirectories, getTopLevelDirectories } = require('../utils/fileScanner');
...
async function analyzeCommand() {
  ...
  if (!isGit) {
    console.log(chalk.yellow('⚠️  Warning: Not a Git repository. Some features may be limited.\n'));
  }
  ...
  console.log(chalk.white(`Total Files: ${chalk.green(scanResults.files.length)}`));
  console.log(chalk.white(`Total Directories: ${chalk.green(scanResults.directories.length)}`));
  ...
  if (hasPackage) {
    ...
    console.log(chalk.white(`Total Dependencies: ${chalk.green.bold(depSummary.totalCount)}`));
    console.log(chalk.white(`  • Dependencies: ${chalk.green(depSummary.dependencyCount)}`));
    console.log(chalk.white(`  • Dev Dependencies: ${chalk.green(depSummary.devDependencyCount)}`));
    
    if (depSummary.peerDependencyCount > 0) {
      console.log(chalk.white(`  • Peer Dependencies: ${chalk.green(depSummary.peerDependencyCount)}`));
    }
    ...
    if (depSummary.dependencies.length > 0) {
      ...
      if (depSummary.dependencies.length > 10) {
        console.log(chalk.gray(`  ... and ${depSummary.dependencies.length - 10} more`));
      }
    }
    ...
    if (depSummary.devDependencies.length > 0) {
      ...
      if (depSummary.devDependencies.length > 10) {
        console.log(chalk.gray(`  ... and ${depSummary.devDependencies.length - 10} more`));
      }
    }
  } else {
